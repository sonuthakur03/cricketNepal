...
import { HiOutlinePlus, HiOutlinePencil, HiOutlineTrash, HiOutlineChartBar } from 'react-icons/hi'
...
export default function SellerDashboard() {
  ...
  useEffect(() => { fetchProducts(); fetchOrders() }, [page])
  ...
  const fetchOrders = async () => {
    try {
      const { data } = await api.get(`/orders/seller-orders?page=${page}&limit=10`)
      setOrders(data.data)
  ...
  return (
    <div className="page-container pt-24 py-12 min-h-screen">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-black" style={{ fontFamily: 'Syne, sans-serif' }}>Seller Dashboard</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <StatCard label="Total Products" value={products.length} icon="📦" color="Active" />
        <StatCard label="Total Orders" value={revenue.totalOrders} icon="🛒" color="Orders" />
        <StatCard label="Total Revenue" value={formatPrice(revenue.totalRevenue)} icon="💰" color="Revenue" />
            ...
            <div className="card overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-[var(--color-border)]">
                  {products.length === 0 ? (
                    <tr><td colSpan={6} className="text-center py-12 text-[var(--color-text-muted)]">No products yet. <Link to="/seller/products/new" className="text-primary-600 font-semibold">Add your first product →</Link></td></tr>
                  ) : products.map((p) => (
                    <tr key={p._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-semibold line-clamp-1 max-w-xs">{p.name}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-[var(--color-text-muted)]">{p.category}</td>
                      <td className="px-4 py-3 font-semibold text-primary-600">{formatPrice(p.discountPrice || p.price)}</td>
                      <td className="px-4 py-3">
                        <span className={p.stock > 0 ? 'badge-green' : 'badge-red'}>{p.stock}</span>
                      </td>
                      <td className="px-4 py-3">⭐ {p.rating?.toFixed(1)} ({p.numReviews})</td>
                      <td className="px-4 py-3">
            ...
            <>
              <div className="card overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-[var(--color-border)]">
                    {orders.length === 0 ? (
                      <tr><td colSpan={6} className="text-center py-12 text-[var(--color-text-muted)]">No orders yet</td></tr>
                    ) : orders.map((o) => {
                      ...
                      return (
                        <tr key={o._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                          ...
                          <td className="px-4 py-3 text-[var(--color-text-muted)]">{formatDate(o.createdAt)}</td>
                          ...
                          <td className="px-4 py-3"><span className={o.isPaid ? 'badge-green' : 'badge-gold'}>{o.isPaid ? 'Paid' : 'Pending'}</span></td>
                        </tr>
                      )
                    })}
                  </tbody>
