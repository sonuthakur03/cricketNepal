// src/pages/ResetPasswordPage.jsx — Premium dark redesign

import { useState } from 'react'
import { useParams, Link, useNavigate } from 'react-router-dom'
import { motion } from 'framer-motion'
import { HiOutlineLockClosed, HiOutlineEye, HiOutlineEyeOff, HiOutlineCheckCircle, HiArrowLeft } from 'react-icons/hi'
      ...
      >
        <div className="text-center mb-8">
          <Link to="/" className="inline-flex flex-col items-center gap-3 mb-6">
            ...
            <span className="text-2xl font-bold" style={{ fontFamily: 'var(--font-display)', letterSpacing: '-0.03em', color: 'var(--text-primary)' }}>
          ...
          <p className="text-sm" style={{ color: 'var(--text-muted)' }}>Choose a new secure password for your account</p>
        </div>
        ...
        <div className="card-glass p-7 rounded-2xl" style={{ border: '1px solid var(--border)', boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }}>
          {success ? (
            <div className="text-center space-y-4 py-4">
              ...
              <h2 className="text-lg font-bold" style={{ fontFamily: 'var(--font-display)', color: 'var(--text-primary)' }}>Password Updated!</h2>
              <p className="text-sm" style={{ color: 'var(--text-muted)' }}>Your password has been reset. You're now logged in.</p>
              <Link to="/" className="btn-primary w-full inline-block text-center mt-4">Go to Homepage →</Link>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  <HiOutlineLockClosed className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: 'var(--text-muted)' }} />
                  <input type={showPw ? 'text' : 'password'} value={password}
                    onChange={(e) => setPassword(e.target.value)}
                    placeholder="Min. 6 characters" className="input pl-10 pr-10" required minLength={6} autoFocus />
              ...
              <div className="text-center">
                <Link to="/login" className="inline-flex items-center gap-1.5 text-sm" style={{ color: 'var(--gold-400)', fontFamily: 'var(--font-display)' }}>
