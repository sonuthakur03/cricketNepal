    ...
    <footer className="bg-dark-900 text-slate-300 mt-20">
      ...
      <div className="bg-primary-600">
        <div className="page-container py-4 flex flex-wrap items-center justify-between gap-4">
          ...
          <Link to="/products" className="bg-white text-primary-700 font-bold text-sm px-5 py-2 rounded-xl hover:bg-primary-50 transition-colors"
      ...
      <div className="page-container py-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        <div>
          <Link to="/" className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xl font-bold text-white" style={{ fontFamily: 'Syne, sans-serif' }}>
              Cricket<span className="text-primary-400">Nepal</span>
            </span>
          </Link>
          <p className="text-sm text-slate-400 leading-relaxed mb-5">
            Nepal's premier destination for cricket equipment, gear, and accessories. Trusted by players across all 7 provinces.
          </p>
        ...
        {/* Shop */}
        <div>
          <h3 className="text-white font-bold mb-4" style={{ fontFamily: 'Syne, sans-serif' }}>Shop</h3>
          <ul className="space-y-2.5">
            {SHOP_LINKS.map((l) => (
              <li key={l.to}>
                <Link to={l.to} className="text-sm text-slate-400 hover:text-primary-400 transition-colors">
                  {l.label}
                </Link>
              </li>
            ))}
        ...
        {/* Company */}
        <div>
          <h3 className="text-white font-bold mb-4" style={{ fontFamily: 'Syne, sans-serif' }}>Company</h3>
          <ul className="space-y-2.5">
            {COMPANY_LINKS.map((l) => (
              <li key={l.to}>
                <Link to={l.to} className="text-sm text-slate-400 hover:text-primary-400 transition-colors">
        ...
        {/* Contact */}
        <div>
          <h3 className="text-white font-bold mb-4" style={{ fontFamily: 'Syne, sans-serif' }}>Contact</h3>
          ...
          <div className="mt-5">
            ...
            <div className="flex gap-2 flex-wrap">
              {['Khalti', 'eSewa', 'COD'].map((p) => (
                <span key={p} className="px-2.5 py-1 bg-dark-800 rounded-lg text-xs text-slate-300 border border-dark-700">
                  {p}
                </span>
              ))}
            </div>
      ...
      <div className="border-t border-dark-800">
        <div className="page-container py-4 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-500">
          <p>© {new Date().getFullYear()} CricketNepal. All rights reserved.</p>
