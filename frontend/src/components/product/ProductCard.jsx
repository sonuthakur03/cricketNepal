    ...
    >
      <Link to={`/products/${product.slug || product._id}`} className="block">
        ...
        <div className="relative overflow-hidden bg-slate-100 dark:bg-slate-800 aspect-square">
          <img
            src={imgError ? 'https://placehold.co/400x400/e2e8f0/94a3b8?text=No+Image' : product.images?.[0]?.url}
            alt={product.name}
          ...
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all duration-300 flex items-center justify-center">
            <div className="flex gap-2 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
              <Link to={`/products/${product.slug || product._id}`}
                className="w-9 h-9 bg-white dark:bg-dark-800 rounded-xl flex items-center justify-center shadow-md hover:bg-primary-50 transition-colors">
        ...
        <div className="p-4">
          ...
          <h3 className="font-semibold text-sm leading-snug line-clamp-2 mb-2 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors"
